...
1. 默认的方法是使用名为 [`pandoc-citeproc`](https://github.com/jgm/pandoc-citeproc) 的 Pandoc 帮助程序，它遵循 [引用样式语言 (Citation Style Language, CSL)](http://docs.citationstyles.org/en/1.0.1/specification.html) 的规范，并从大量可用的 [CSL 样式文件 (CSL style files)](https://www.zotero.org/styles/) 之一中获取特定的格式说明。
...
&emsp;&emsp;参考书目条目以 `@type{` 开头，其中 `type` 可以是 `article`、`book`、`manual`等等。^[类型名称不区分大小写，因此不管是 `manual`、`Manual` 或 `MANUAL` 都可以。]紧跟着是一个引文关键词，在上面的示例中为 `R-base`。要引用条目，使用 `@key` 或 `[@key]`（后者将引用文字放在圆括号中），例如，`@R-base` 被渲染为 @R-base，而 `[@R-base]` 则渲染为 "[@R-base]"。如果你对於 LaTeX 中的 **natbib** 软件包很熟悉，你会发现 `@key` 基本上就是 `\citet{key}`，而 `[@key]` 等同于 `\citep{key}`。
...
&emsp;&emsp;然后通过 YAML 选项 `includes -> after_body` 在书的末尾插入 `\printindex`。索引项可以通过书籍正文中的 `\index{}`命令创建，例如 `\index{GIT}`。
...
&emsp;&emsp;第三，**webshot** 有一些控制自动屏幕截图的选项，你可以通过区块选项 `screenshot.opts` 指定这些选项，该选项采用类似 `list(delay = 2, cliprect = 'viewport')` 的列表。有关可用选项的完整列表，请查阅帮助页面 `?webshot::webshot`，而[软件包说明短文](https://cran.rstudio.com/web/packages/webshot/vignettes/intro.html) `vignette('intro', package = 'webshot')` 说明了这些选项的效果。这里的 `delay` 选项对于需要很长时间进行渲染的小组件来说很重要：`delay` 指定了 PhantomJS 在截图前等待的秒数。如果看到不完整的屏幕截图，可能需要指定更长的延迟时间（默认值为 0.2 秒）。
...
```{r miniUI, fig.cap='通过 miniUI 软件包创建的一个 Shiny 应用；你可以在 <https://yihui.shinyapps.io/miniUI/> 看到现场版本', screenshot.opts=list(delay=20,zoom=2), dev='png', cache=TRUE, fig.align='center', fig.width=8, fig.height=6}
